...
const t0 = Date.now();
// Bake the RAW rank mask. The white-CDF remap is applied AT RUNTIME into the
// texture's R channel only (grain engine); the RISO paths (risoMatrixDither,
// gpuGrainTouch, dissolveInk) threshold against UNIFORM ranks in G — baking
// the remapped bytes broke all of them (noise instead of ordered grain-touch).
const mask = genVoidClusterMask(SIZE);
...
// ${SIZE}x${SIZE} void-and-cluster threshold mask, RAW UNIFORM RANKS.
// The grain engine's white-CDF tone remap happens at runtime (R channel);
// RISO paths threshold these raw ranks from the G channel.
